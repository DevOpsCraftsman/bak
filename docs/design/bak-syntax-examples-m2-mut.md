# Bak Language — Syntax Examples (M2: `:=` + `mut`)

Complete reference using the **M2 syntax**: `:=` immutable by default, `mut` for
mutability (Rust-inspired), `fn` keyword, no colon in type annotations.
...
// when — for conditional branching
